...
## 14. 全站日期显示 -1 天的时区 bug（2026-09-26 发现并修复）

**症状**：线上文章头部/列表卡片的日期比 frontmatter 早一天——W39 页头部「每周复盘 2026/09/23、数据截至 2026/09/23」（应为 09/24）；W38 页头部「2026/09/17」（应为 09/18）。正文关键事实表里的日期是**原样字符串**（如「2026-09-24 收盘」），不受影响。

**根因**：`src/lib/content.ts` 的 `formatDate` 用 `Intl.DateTimeFormat("zh-CN")` **未指定 `timeZone`**。它把 `date T00:00:00+08:00` 构造成绝对时刻后，再按**运行环境的本地时区**格式化——本机（UTC+8）显示正确，**Vercel 构建机是 UTC**，`2026-09-24T00:00+08` = `2026-09-23T16:00Z` → 显示 `2026/09/23`。所以本地构建怎么测都正常、只有线上偏一天。

**修复**：`formatDate` 显式加 `timeZone: "Asia/Shanghai"`（全站唯一一处 `Intl.DateTimeFormat`，一处修复覆盖全部日期渲染）。`<time datetime>` 原始属性与 sitemap 用的都是原始日期串，**一直是对的**（SEO 未受影响），坏的只是显示文本。

**排查教训**：
1. 本地构建正常 ≠ 线上正常——时区类 bug 只在 UTC 构建机暴露，核验日期必须以**线上**为准。
2. `WebFetch` 摘要会串味日期（0924 先例），本轮 W39 头部 09/23 一度被误判为摘要串味——判别方法是先确认「构建逻辑上能否出现该值」（MDX 无 09-23、远端=本地），再用 `web_reader`（保留原文）拿独立证据，最后用**另一篇已上线文章（W38）交叉验证**：它的 frontmatter date 09-18 与线上显示 09/17 的系统性 -1 天，即坐实是代码 bug 而非抓取噪声。
