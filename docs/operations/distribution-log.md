...
| 2026-W39 | weekly | **已发布**（2026-09-26 建 MDX 并部署，status public） | 草稿 | 未发（paused） | 中秋短周周稿（4 个交易日，09-25 休市）；雪球海报用**星云随笔**背景（周稿默认约定，深底 `--theme dark`）；题眼「冲高回落收阴 / 小盘还在撑着」；估值栏回填 revision 242（数据截止 09-24，口径完整；线上 ERP 核对一致 5.82）、三段文末追加 revision 243（diff 1 hunk / +23−0）；MDX `weekly/2026-W39` 已建、`npm run check` 0 错误、`npm run build` 通过（/weekly/2026-W39/，全站 **34 页**）；雪球草稿按周稿口径生成；交接包 ready（poster SHA `124b0e66…`、底图 SHA 与画布库一致）；首页与 /weekly/ 列表页已切至 W39；**本行起按 2026-09-26 用户定版程序执行：海报目检「确认」后直接完成 B 侧（已回写 skill §5.5）** |
...
| — | 全部 | — | — | — | **2026-09-26 时区 bug 修复（随 W39 上线发现）**：`formatDate`（`src/lib/content.ts`）未指定 `timeZone`，Vercel 的 UTC 构建机把全站文章头部/列表日期渲染成 **-1 天**（W38 线上头部 09/17 应为 09/18、W39 09/23 应为 09/24）；`<time datetime>` 属性与 sitemap 一直正确，仅显示文本受影响。修复 = 显式 `timeZone: "Asia/Shanghai"`（全站唯一 Intl 调用点），重建部署后线上核验 W39 = 09/24。详见 runbook §14 |
