# 雪球发布版 · 2026-09-04 周度复盘（W36，分发草稿）

> 状态：draft（待用户确认后发布）
> 来源：博客 MDX + `content-inbox/2026-W36-weekly/handoff.md`（已确认终稿，飞书 revision 339）
> 用途：雪球发布 = 主图海报 + 突出重点短评（完整长文在个人博客）；题眼/摘要/边界口径逐字取自博客，不新增、不改义。
> 发布时：上传 `poster-2026w36.png`（1080×2000 复盘海报）；正文尾部已附博客入口（当前为内网临时地址，真实域名上线后替换）。
